...
cd /home/ubuntu/django-aws_cicd/review_service/
...
sed -i "/ALLOWED_HOSTS/c\ALLOWED_HOSTS = ['$(curl -s ifconfig.me)']" ./review_service/settings.py

# run server
python3 manage.py makemigrations api_v1
python3 manage.py migrate
screen -d -m python3 manage.py runserver 0:8004 > output.txt 2>&1
